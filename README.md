...
| keyCodeConfig | <code>interface KeyCodeConfig { <br>  firstSlide?: number[]; <br>  lastSlide?: number[];<br>  nextSlide?: number[]; <br>  pause?: number[]; <br>  previousSlide?: number[]; <br>}</code> | If `enableKeyboardControls` prop is true, you can pass configuration for the keyCode so you can override the default keyboard keys configured. | `{ nextSlide: [39, 68, 38, 87], previousSlide: [37, 65, 40, 83], firstSlide: [81], lastSlide: [69], pause: [32] }` |
