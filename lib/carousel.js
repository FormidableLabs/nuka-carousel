...
var _initialiseProps = function _initialiseProps() {
  ...
  this.getSlideTargetPosition = function (index, positionValue) {
    var slidesToShow = _this3.state.frameWidth / _this3.state.slideWidth;
    var targetPosition = (_this3.state.slideWidth + _this3.props.cellSpacing) * index;
    var end = (_this3.state.slideWidth + _this3.props.cellSpacing) * slidesToShow * -1;

    if (_this3.props.wrapAround) {
      var slidesBefore = Math.ceil(positionValue / _this3.state.slideWidth);
      if (_this3.state.slideCount - slidesBefore <= index) {
        return (_this3.state.slideWidth + _this3.props.cellSpacing) * (_this3.state.slideCount - index) * -1;
      }

      var slidesAfter = Math.ceil((Math.abs(positionValue) - Math.abs(end)) / _this3.state.slideWidth);

      if (_this3.state.slideWidth !== 1) {
        slidesAfter = Math.ceil((Math.abs(positionValue) - _this3.state.slideWidth) / _this3.state.slideWidth);
      }

      if (index <= slidesAfter - 1) {
        return (_this3.state.slideWidth + _this3.props.cellSpacing) * (_this3.state.slideCount + index);
      }
    }
